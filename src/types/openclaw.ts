...
/** Totals for cost + tokens across a range. */
...
/** One per UTC day — `usage.cost` response. */
export interface UsageCostDailyEntry {
...
export interface SessionsUsageDailyBreakdown {
  date: string;
  tokens: number;
  cost: number;
}

export interface SessionsUsageDailyMessageCount {
  date: string;
  total: number;
  user: number;
  assistant: number;
...
export interface SessionsUsageDailyLatency {
  date: string;
  count: number;
  avgMs: number;
  p95Ms: number;
...
export interface SessionsUsageDailyModelUsage {
  date: string;
  provider: string;
  model: string;
  tokens: number;
...
/** Per-session usage detail embedded inside `sessions.usage` response. */
export interface SessionsUsageDetail {
  sessionId: string;
  sessionFile?: string;
  ...
  dailyBreakdown: SessionsUsageDailyBreakdown[];
  dailyMessageCounts: SessionsUsageDailyMessageCount[];
  dailyLatency: SessionsUsageDailyLatency[];
  dailyModelUsage: SessionsUsageDailyModelUsage[];
  totalTokens?: number;
  totalCost?: number;
}
...
/** One point in `sessions.usage.timeseries` response. */
