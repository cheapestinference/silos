...
function JsonTree({ data, depth = 0 }: { data: unknown; depth?: number }) {
  ...
  if (typeof data === 'string') {
    // Long strings get special treatment
    if (data.length > 120) {
...
export function LogDetailPanel({ line, onClose }: LogDetailPanelProps) {
  ...
  return (
    <div className="w-[420px] shrink-0 border-l border-gray-800 flex flex-col bg-[#0a0e14]">
      {/* Header */}
      ...
      {ts && (
        <div className="px-4 py-2 border-b border-gray-800/50">
          ...
          <span className="text-[10px] text-gray-400 font-mono tabular-nums">{ts}</span>
        </div>
      )}
      ...
      {line.message && (
        <div className="px-4 py-2.5 border-b border-gray-800/50">
          <p className="text-[10px] text-gray-600 mb-1">Message</p>
          <p className="text-xs text-gray-300 whitespace-pre-wrap break-words leading-relaxed">{line.message}</p>
        </div>
      )}

      {/* JSON tree */}
      <div className="flex-1 overflow-y-auto px-4 py-3">
        <p className="text-[10px] text-gray-600 mb-2">Payload</p>
        <div className="font-mono text-[11px] leading-relaxed">
          {line.payload ? (
            <JsonTree data={line.payload} />
          ) : (
            <pre className="text-gray-400 whitespace-pre-wrap break-words">{line.raw}</pre>
          )}
        </div>
      </div>
    </div>
